...
const VapiChatAssistant = ({ inline = false }) => {
      ...
      {(isOpen || inline) && (
        <div className={`vapi-chat-window ${inline ? 'inline' : ''}`}>
          <div className="vapi-chat-header">
            <h3>🤖 {inline ? 'Interactive AI Assistant' : 'KrishiSetu Voice Agent'}</h3>
          ...
          <div className="vapi-chat-controls">
            ...
            <form onSubmit={handleSendText} className="text-form">
              <input
                ...
                placeholder={isCallActive ? "Type here..." : "Turn on Voice to start chatting"}
