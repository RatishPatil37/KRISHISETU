  ...
  try {
    ...
    try {
      ...
      geminiData = {
        ...
        description: `Crop identified as ${cropName}. Condition: ${condition}. (Note: AI detailed analysis is currently limited: ${cleanErr})`,
...
router.post('/generate-pdf', async (req, res) => {
  try {
    ...
    const drawFooter = (d) => {
      ...
      d.rect(0, fy, PW, FOOTER_H).fill(GREEN_DARK);
      d.fillColor('#a7f3d0').font('Helvetica').fontSize(7.5)
        .text(
      ...
      d.fillColor(WHITE).font('Helvetica-Bold').fontSize(9)
        .text(
    ...
    const drawHeader = (d) => {
      d.rect(0, 0, PW, 95).fill(GREEN_DARK);
      d.fillColor('#a7f3d0').font('Helvetica').fontSize(7.5)
        .text('KRISHISETU  |  AI CROPS DOCTOR  |  CROP HEALTH ASSESSMENT REPORT', MARGIN, 12, { width: COL, align: 'center' });
      d.rect(MARGIN, 23, COL, 0.5).fill('#2d7a47');
      d.fillColor(WHITE).font('Helvetica-Bold').fontSize(20)
        .text('AI Crops Doctor', MARGIN, 32, { width: COL, align: 'center' });
      d.fillColor('#bbf7d0').font('Helvetica').fontSize(9.5)
        .text('Powered by Custom Crop_Model.pt (24-Class AI) & Gemini AI', MARGIN, 57, { width: COL, align: 'center' });
      const rDate = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
      const rTime = new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
      d.fillColor('#6ee7b7').font('Helvetica').fontSize(8)
        .text(`Report Date: ${rDate}  at  ${rTime}`, MARGIN, 75, { width: COL, align: 'center' });
    };
    ...
    // (already drawn by pageAdded or initial call)
    doc.y = 110;
    ...
    if (imageBase64) {
      try {
        ...
        doc.image(Buffer.from(pureBase64, 'base64'), MARGIN, sectionY, { width: IMG_SZ, height: IMG_SZ, fit: [IMG_SZ, IMG_SZ] });
      } catch (err) {
        console.error('[PDF] Image insertion failed:', err.message);
    ...
    if (analysis.allDiseases && analysis.allDiseases.length > 0) {
      ...
      doc.text('Confidence', MARGIN + COL * 0.62 + 8, tY + 5, { width: COL * 0.38 - 16, align: 'right' });
      doc.y = tY + ROW;

      analysis.allDiseases.forEach((d, i) => {
        ensureSpace(ROW + 2);
        const ry = doc.y;
        if (i % 2 === 0) doc.rect(MARGIN, ry, COL, ROW).fill('#f9fafb');
        doc.fillColor(GREY_DARK).font('Helvetica').fontSize(8.5)
          .text(d.name, MARGIN + 8, ry + 5, { width: COL * 0.62 });
        doc.fillColor(GREEN_MID).font('Helvetica-Bold').fontSize(8.5)
          .text(`${d.confidence}%`, MARGIN + COL * 0.62 + 8, ry + 5, { width: COL * 0.38 - 16, align: 'right' });
        doc.y = ry + ROW;
      });
    }
